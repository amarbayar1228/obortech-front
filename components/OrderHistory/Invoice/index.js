...
return <div>
<Button type="primary" size="small" onClick={showModal}>Invoice</Button>
<Modal title="Invoice" width={630} open={isModalOpen} onOk={handleOk} onCancel={handleCancel} footer={null}>
        ...
        <div className={css.InvoiceLogo}>
            <Image alt="Obertech" preview={false} style={{position: "relative",width: "120px",height: "50px",objectFit: "inherit"}}src="/img/OBORTECH_logo_H_clean.svg"/>
        </div>
            <div> Invoice number - 0037038387</div>
        </div>
        <div className={css.InvoiceTable}>
        <div className={css.Contract1}>
            <div className={css.InvoiceDate}>{dateState} - Tulburiin nehemjlel</div>
            <div className={css.InvoiceLayout}> <div> Hereglegch ner: </div> <div> 9011826614 BEC</div></div>
            <div className={css.InvoiceLayout}><div>Gereenii dugaar: </div><div> 9011826614</div></div>
            <div className={css.InvoiceLayout}><div>Date: </div><div>{dateState} - {datePlusState}</div>
            </div>
        </div>
        <div className={css.Contract2}>
            ...
            <div className={css.Sector}>
                <div className={css.SectorText}>
                    <div className={css.IconCaretRight}><CaretRightOutlined /></div><div>Үйлчилгээний салбар</div>
                </div>
                <div className={css.SectorText}><div className={css.IconCaretRight}><CaretRightOutlined /></div><div> Банкны данс</div></div>
            </div>
            </div>
            <div className={css.ContractLayout}>
            ...
            <div>
                ...
                <div className={css.ItemLayout} key={i}>
                    ...
                    <div style={{width: "20%",borderRight: "1px solid #ccc",}}>Qty: {e.cnt}</div>
                    <div className={css.ContractItemTotalBorder}>{e.price.toFixed(1).replace(/\d(?=(\d{3})+\.)/g, "$&,")}$</div>
                </div>
                ))}
            </div>
            ...
            <div className={css.ItemLayout}>
                ...
                <div className={css.ContractItemTotalBorder} style={{fontWeight: "700"}}>{totalPrice.toFixed(1).replace(/\d(?=(\d{3})+\.)/g, "$&,")}$</div>
            </div>
            </div>
            <div className={css.ContractLayout}> 
            <div className={css.PayState}> Нэхэмлэх дүн</div>
            <div className={css.ItemLayout}>
                <div className={css.ContractItemText}>Татвар тооцоогүй дүн:</div>
            ...
            <div className={css.ItemLayout}>
                <div className={css.ContractItemText}>Нийт төлбөл зохих дүн:</div>
                <div className={css.ContractItemTotalBorder}>4,555,00 </div>
            </div>
            </div>
        </div>
